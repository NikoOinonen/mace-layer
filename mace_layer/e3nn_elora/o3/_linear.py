        ...
        *,
        ...
        if internal_weights and self.weight_numel > 0:
            ...
            if self.r_lora is not None:
                ...
                self.LoRA_weight = []
                self.alpha = self.r_lora
                for ins in self.instructions:
                    if ins.i_in != -1:
                        self.LoRA_weight.append(torch.nn.Parameter(torch.randn(*((f_in, f_out) if f_in is not None else ()), ins.path_shape[0], self.r_lora)))
                        self.LoRA_weight.append(torch.nn.Parameter(torch.zeros(*((f_in, f_out) if f_in is not None else ()), self.r_lora, ins.path_shape[1])))
                self.LoRA_weight = torch.nn.ParameterList(self.LoRA_weight)
                self.LoRA_weight_numel = sum(ins.path_shape[0] * self.r_lora + self.r_lora * ins.path_shape[1] for ins in instructions if ins.i_in != -1)
            else:
    ...
    def forward(self, features, weight: Optional[torch.Tensor] = None, bias: Optional[torch.Tensor] = None):
        ...
        if weight is None:
            ...
            if self.r_lora is not None:
                ...
                weight = weight + self.alpha / self.r_lora * torch.cat(LoRA_weight, dim=-1)
        if bias is None:
    ...
    def merge_LoRA(self):
        ...
        for index, v in enumerate(self.LoRA_weight):
            if index % 2 == 0:
                ...
                LoRA_weight[-1] = (LoRA_weight[-1] @ v).flatten()
        self.weight.data = self.weight + self.alpha / self.r_lora * torch.cat(LoRA_weight, dim=-1)
